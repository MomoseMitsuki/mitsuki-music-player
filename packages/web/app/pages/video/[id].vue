...
import MockVideoData from "~/mock/video.json";
import { eventEmitter } from "#imports";
...
async function initVideo(url: string) {
	await nextTick;
	if (player) {
		player.url = url;
		return;
	...
	player = new Artplayer({
		container: videoRef.value as HTMLDivElement,
		url,
		theme: "#ed448e",
		volume: 0.5,
		isLive: false,
...
onUnmounted(() => {
	...
	eventEmitter.off("MUSIC:PLAY", stopPlayer);
});
</script>
